...
import { Bug, Data, FileExcel, LayoutThree, Lightning, Link, Share } from '@icon-park/vue-next'
import { h, computed } from 'vue'
import { useRoute } from 'vue-router'

const envStore = useEnvStore() // debug store
const route = useRoute()

const activeLink = computed(() => route.path) // 获取激活路径
  ...
  <div class="app-navigator">
    ...
    <div class="app-navigator-link-wrapper">
      <router-link
        ...
        :key="item.title"
        :to="item.link"
        :style="activeLink.includes(item.link) ? { background: item.bg } : {}"
        class="app-navigator-link-item"
      >
        <div
          :style="{
            lineHeight: 0.7,
            color: activeLink.includes(item.link) ? item.color : 'var(--color-gray-700)',
          }"
        >
        ...
        <div
          class="item-border"
          :style="activeLink.includes(item.link) ? { background: item.borderColor } : {}"
        ></div>
      </router-link>
    </div>
